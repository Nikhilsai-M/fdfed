    ...
    <header className="fixed top-0 left-0 w-full bg-white shadow-md z-50">
      ...
      <div className="flex justify-between items-center px-4 py-2 md:px-5">
        <a href="/" className="logo-container block">
          <img src="/images/header/logo1.png" alt="Logo" className="w-32 md:w-44" />
        </a>
        ...
        <div className="flex items-center gap-4 md:gap-6">
          <a href={session?.loggedIn ? '/cart' : '/login'} className="flex items-center text-black hover:text-blue-500 transition-colors duration-200">
            <img src="/images/header/cart-icon.png" alt="Cart" className="cart-icon w-6 h-6 mr-1" />
            <span className="text-sm md:text-base">Cart</span>
          </a>

          <div className="relative group">
            <a href={session?.loggedIn ? '/profile' : '/login'} className="flex items-center text-black hover:text-blue-500 transition-colors duration-200">
              <img src="/images/header/profile-icon.png" alt="Profile" className="profile-icon w-6 h-6 mr-1 hidden md:block" />
      ...
      <nav className="bg-blue-500 text-white px-4 py-2 flex justify-between items-center md:justify-center">
        ...
        <div className={`md:flex md:gap-x-2 ${isMenuOpen ? 'flex flex-col absolute top-full right-0 bg-white text-black w-48 shadow-md rounded-md z-50' : 'hidden md:flex'}`}>
          ...
          <div className="relative group">
            ...
            <div className="dropdown-menu absolute top-full left-0 bg-white text-black shadow-lg rounded-lg hidden group-hover:block w-48 border border-gray-200 overflow-hidden">
              <a href="/Accessories?category=smartwatches" className="dropdown-item block px-4 py-2.5">Smart Watches</a>
              <a href="/Accessories?category=earbuds" className="dropdown-item block px-4 py-2.5">Ear Phones</a>
              <a href="/Accessories?category=chargers" className="dropdown-item block px-4 py-2.5">Chargers</a>
              <a href="/Accessories?category=mouses" className="dropdown-item block px-4 py-2.5">Mouses</a>
            </div>
          </div>
